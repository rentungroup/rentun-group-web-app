...
export const fetchConfig = async () => {
  try {
    ...
    const mappedProperties = propertiesRes.data?.map(p => ({
      ...
      airbnbBooking: (p.airbnb_booking && p.airbnb_booking.includes('book/stays')) ? p.airbnb_listing : p.airbnb_booking,
      airbnbReviews: p.airbnb_reviews,
  ...
  try {
    ...
    if (newData.properties) {
      ...
      const propData = newData.properties.map(p => ({
        ...
        airbnb_listing: p.airbnbListing || '',
        airbnb_booking: (p.airbnbBooking && p.airbnbBooking.includes('book/stays')) ? p.airbnbListing : p.airbnbBooking,
